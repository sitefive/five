    ...
    try {
      const { data, error } = await supabase
        ...
        .select(`
          ...
          published_at,
          post_tags:post_tags(tag:tags(name_pt))
        `)
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <Link to="/admin/posts/new" className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 flex items-center">
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filteredPosts.map((post) => (
              <tr key={post.id}>
                <td className="px-6 py-4"><Link to={`/admin/posts/${post.id}`} className="text-sm font-medium text-blue-600 hover:underline">{post.title_pt || t('common.no_title_fallback')}</Link></td>
                ...
                <td className="px-6 py-4 text-sm text-gray-500">
                    {post.post_tags?.map(pt => pt.tag.name_pt).join(', ')}
                </td>
