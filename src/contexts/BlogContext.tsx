...
export function BlogProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
        ...
        if(debouncedSearchQuery) {
            query = query.textSearch(`fts_${lang}`, debouncedSearchQuery); // Supondo que você tenha uma coluna 'fts' para Full-Text Search
        }
  ...
  const loadMorePosts = useCallback(async () => {
    ...
    if(debouncedSearchQuery) {
        query = query.textSearch(`fts_${lang}`, debouncedSearchQuery);
    }
